...
module.exports = {
  ...
  //  get skill object by reference to name or abbr
  getSkill: function getSkill(skill) {
    var skillObj = null;
  ...
  //  Return HTML of skill name with Logo image beside it
  ...
  //  Return the age in years if given a dateString of the format 'YYYY-MM-DD' (optionally padded with zeros)
  getAge: function getAge(dateString, padZeros, currentDate) {
  ...
  disguiseMail: function disguiseMail(mail) {
    return mail.replace('@','&#64;').replace('o','&#111;');
  },
  //  checks if IP is on the blacklist
  isBlacklistedIP: function isBlacklistedIP(ip, callback) {
    var request_get = require('request').get;
    request_get('http://jeremymoritz.com/support/ip_blacklist.json', function reviewBlacklist(err, res, json) {
      if (err) { console.log(err); return; }
      if (res.statusCode !== 200) { console.log('STOP! IP Blacklist status code: ' + res.statusCode); return; }

      var blacklist = JSON.parse(json).blacklist;
      var ipIsBlacklisted = false;
      _.forEach(blacklist, function eachIP(blackIP) {
        if(ip === blackIP) {
      ...
      if (typeof callback === 'function') {
        callback(ipIsBlacklisted);  //  execute callback and pass in the status of the ip
      }
    });
  }
};
